...
def build_markdown_decision_prompt(sku: pd.Series) -> str:
    ...
    velocity = float(sku.get("sales_velocity", 0))
    days     = min(float(sku.get("days_of_stock", 9999)), 9999)
    risk     = sku.get("clearance_risk", "LOW")
    abc      = sku.get("abc_class", "C")

    # Pre-compute suggested markdown based on rules
    # so Groq gets a filled example and never returns brackets
    if abc == "A" and risk == "LOW":
        suggested_md = 5
    elif risk == "HIGH" and days > 365:
        suggested_md = 35
    elif risk == "HIGH":
        suggested_md = 25
    elif risk == "MEDIUM" and days > 180:
        suggested_md = 20
    elif risk == "MEDIUM":
        suggested_md = 15
    elif velocity < 0.01 and stock > 100:
        suggested_md = 30
    else:
        suggested_md = 5

    suggested_price  = round(price * (1 - suggested_md / 100), 2)
    suggested_margin = round((suggested_price - cost) / suggested_price * 100, 1) if suggested_price > 0 else 0
    curr_margin      = round((price - cost) / price * 100, 1) if price > 0 else 0

    # Pick suggested strategy
    if abc == "A" and risk == "LOW":
        suggested_strategy = "Loyalty Member Exclusive"
    elif risk == "HIGH" and days > 365:
        suggested_strategy = "Clearance Markdown"
    elif risk == "HIGH":
        suggested_strategy = "Flash Sale"
    elif risk == "MEDIUM":
        suggested_strategy = "Moderate Markdown"
    else:
        suggested_strategy = "No Action Needed"

    suggested_risk = {"HIGH": "High", "MEDIUM": "Medium", "LOW": "Low"}.get(risk, "Medium")
...
- Product Name:      {sku.get('product_name')}
- Category:          {sku.get('main_category')}
- ABC Class:         {abc}  (A=top revenue, B=mid, C=low)
- Current Price:     ${price:.2f}
- Cost Price:        ${cost:.2f}
...
Pre-computed suggestion based on rules above (you may override with better reasoning):
RECOMMENDED_MARKDOWN: {suggested_md}
NEW_PRICE: {suggested_price}
STRATEGY: {suggested_strategy}
CONFIDENCE: 80
REASONING: Stock has {days:.0f} days of coverage with {risk} clearance risk. Velocity is {velocity:.6f} units/day. A {suggested_md}% markdown is suggested to improve sell-through while keeping margin at {suggested_margin}%.
MARGIN_AFTER: {suggested_margin}
RISK_LEVEL: {suggested_risk}

Now respond with your own expert analysis. Write ONLY the actual value after each colon — no brackets, no placeholder text, no extra explanation outside the format below:
RECOMMENDED_MARKDOWN: {suggested_md}
NEW_PRICE: {suggested_price}
STRATEGY: {suggested_strategy}
CONFIDENCE: 80
REASONING: Your 3-4 sentence reasoning here based on the product data above.
MARGIN_AFTER: {suggested_margin}
RISK_LEVEL: {suggested_risk}
"""
